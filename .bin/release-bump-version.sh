...
# mettre à jour readme.txt + le fichier principal, committer et pousser.
# Ne crée PAS de tag — voir release-tag.sh pour ça.
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  --apply     : effectue réellement les changements (fichiers + commit + push)."
            echo ""
            echo "Ne crée pas de tag. Utiliser release-tag.sh une fois le commit poussé pour créer et pousser le tag."
            exit 0
            ;;
        *)
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║         RELEASE — BUMP VERSION                    ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════════╝${NC}"
echo ""
log_info "Plugin : $dir_name"
log_info "Répertoire : $PLUGIN_DIR"
if [[ "$APPLY" == true ]]; then
    log_warning "Mode APPLY activé — les fichiers seront modifiés, committés et poussés."
else
...
if ! git -C "$PLUGIN_DIR" rev-parse --git-dir &>/dev/null; then
    log_error "Pas un repo git — pas de commit possible, abandon"
    exit 1
fi
...
log_step "Push de la branche $current_branch…"
if git -C "$PLUGIN_DIR" push origin "$current_branch"; then
...
echo ""
separator
log_success "Bump de version terminé pour $dir_name (nouvelle version : $new_version)"
log_info "Lancez release-tag.sh pour créer et pousser le tag."
